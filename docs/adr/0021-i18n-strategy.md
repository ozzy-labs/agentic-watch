...
- **セッション自身のナレーション / ログ / 推論の locale 追従は opt-in**（[#382](https://github.com/ozzy-labs/feedradar/issues/382)）。上記は agent の「出力成果物」を locale 化するが、routine セッション**自身の進捗ナレーション・ログ・推論の表出**は既定で英語のまま（英語が最も確実な指示言語で、推論を予測可能に保つため・本 D5 の英語正本プロンプト方針）。これを locale 言語に切り替えたい運用者向けに **`radar routine generate pipeline --localize-session`** を用意する。`--localize-session` 指定かつ locale != en のときのみ `## Hard constraints` にセッション・ナレーション・ディレクティブ（`buildSessionLocaleDirective` / `cli.routine.sessionLocaleDirective`）を追加する。**既定は off**（フラグ未指定または en では何も足さない）で、bootstrap プロンプトの英語正本は不変。opt-in に留めることで「英語推論を既定とする」本 ADR の方針と衝突させない。
