...
- **Host-agent (in-session) mode**: `research` (PoC), `review`, and `update` are **shipped** ([ADR-0019](../adr/0019-host-agent-execution-mode.md)). All three use the prepare/commit 2-call protocol (`--emit-payload` / `--commit`) and share a `finalize*()` primitive with their spawn path, so schema validation and the status transition stay CLI-owned. `review` does in-place dual-update (§7.2, `researched → reviewed`), `update` regenerates a v+1 file (§8.3 invariants, items.yaml status unchanged). `--commit` paths are constrained to `<cwd>/research/` (literal prefix + symlink `realpath`) via the shared `resolveCommitPathInside`. `triage` writes a per-item `TriageDecision` with **no report file** ([ADR-0018](../adr/0018-triage-extension.md)), so its payload/commit contract is a different artifact and remains **deferred**.
