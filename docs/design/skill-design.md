...
- **Host-agent (in-session) mode for `review` / `update` / `triage`**: deferred ([ADR-0019](../adr/0019-host-agent-execution-mode.md)). `research` is the PoC for the prepare/commit 2-call protocol (`--emit-payload` / `--commit`). `review` and `update` are **the same shape** (generate Markdown → finalize), so the same prepare/commit contract applies once their finalize steps — `review`'s atomic dual-update (§7.2) and `update`'s v+1 invariants (§8.3) — are factored into the shared `finalizeResearch()`-style primitive. `triage` writes a per-item `TriageDecision` with **no report file** ([ADR-0018](../adr/0018-triage-extension.md)), so its payload/commit contract is a different artifact and is lower priority. Revisit after the `research` PoC.
