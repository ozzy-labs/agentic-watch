...
| **schedule scaffolds** (opt-in) | `<cwd>/.claude/routines/watch-daily.yaml` / `<cwd>/.github/workflows/watch.yaml` | Templates for connecting to a recurring scheduler ([ADR-0004](../adr/0004-schedule-strategy.md)). | `src/templates/{routines,workflows}/` | (opt-in: `--with-routines` / `--with-actions`) |
